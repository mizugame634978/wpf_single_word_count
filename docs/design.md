...
WPF (.NET) で動作する単一ユーザ向けの英単語学習デスクトップアプリ。
...
判定:
- テキスト入力時は大文字小文字無視・前後空白無視・複数訳は `;` のどれかに
  一致すれば正解 (緩判定)。完全一致モードもオプションで提供
...
### 3.6 LLM 生成 (将来機能, 初期はインタフェースのみ用意)

- ユースケース
  - テーマ (例: "ビジネス英語の動詞") と件数を指定し、新規単語を一括生成
  - 既存単語に対して「より自然な和訳の候補」「例文」を後付けで生成
- 抽象化
  - `IVocabularyGenerator` インタフェースを定義し、実装は差し替え可能に
  - 初期実装は Anthropic Claude API を想定 (`AnthropicVocabularyGenerator`)
  - API キーは Windows Credential Manager に保存
- UX
  - 生成結果はそのまま登録せず、確認画面で取捨選択してから単語帳に追加
  - 重複は自動でスキップ候補としてマーク

> 初期リリースでは UI 上「LLM 生成」ボタンは無効化 (or 非表示) とし、
> インタフェースとサービス層の口だけ用意しておく。
...
| 区分 | 採用 | 備考 |
|------|------|------|
| ランタイム | .NET 8 (LTS) | Windows 専用 (`net8.0-windows`) |
...
| LLM (将来) | Anthropic.SDK (公式 .NET SDK) | 抽象化越しに利用 |
...
| **2. CSV 入出力** | インポート / エクスポート (v1 フォーマット) | 別ツールの CSV を取り込み・書き出しできる |
| **3. テスト機能** | 4 モード + 結果集計 + 習熟度更新 | 自分でテストして進捗が記録される |
| **4. 仕上げ** | フィルタ強化, テーマ, ショートカット, エラーハンドリング | 日常運用に耐える品質 |
| **5. LLM 連携** | `IVocabularyGenerator` 実装 + 生成 → 確認 → 取り込み | テーマ指定で単語が自動追加できる |
...
1. **対象 OS** — Windows 10 / 11 のみで良いか (`net8.0-windows`)。
2. **配布形態** — 自家用なら `dotnet publish` のフォルダ配布で十分か、
   それとも MSIX / インストーラまで作るか。
3. **CSV の列名** — 上記 v1 で問題ないか。既存教材の CSV があるなら
   その列名に揃えたい。
4. **判定の厳密度** — 大文字小文字無視・前後空白無視のデフォルトで OK か。
   ハイフン / ピリオドの扱いは?
5. **LLM プロバイダ** — 初期実装は Anthropic Claude で想定しているが、
   OpenAI / ローカル LLM への切替予定はあるか。
6. **多言語化** — UI は日本語固定で良いか、英語切替も視野に入れるか。

---

> 次のアクション: 上記オープン論点に回答をもらった上で、Phase 0
> (ソリューション雛形 + EF Core 初期マイグレーション) から着手する想定です。
