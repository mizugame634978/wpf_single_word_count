...
判定 (Phase 3 実装で改訂):
- **英→和 (タイピング)**: 自動判定はしない。ユーザがタイプ → Enter で
  正解を露出 → 「○ 正解 / × 不正解」を自己採点 (Anki 風)。日本語訳の表記
  ゆれを機械判定する難易度を回避するための設計
- **和→英 (タイピング)**: 大文字小文字無視・前後空白無視で自動判定。
  英語側は表記ゆれが少ないため自動判定が成立しやすい
- **4 択 (英→和)**: 選択肢のうち単語の `meaning` と完全一致するものが
  正解
- **フラッシュカード**: 表示 → 「答えを見る」 → 自己採点
...
| Phase | 内容 | 状態 |
|-------|------|------|
| **0. 基盤** | ソリューション作成, DI / EF Core / ロガー | ✅ 完了 (PR #1) |
| **1. 単語 CRUD** | 単語の追加 / 編集 / 削除 / 一覧 + タグ | ✅ 完了 (PR #2) |
| **2. CSV 入出力 + LLM プロンプト出力** | インポート / エクスポート + 外部 LLM 用プロンプトテンプレート | ✅ 完了 (PR #3) |
| **3. テスト機能** | 4 モード + 結果集計 + 習熟度更新 + 自己採点 UX | ✅ 完了 (PR #4) |
